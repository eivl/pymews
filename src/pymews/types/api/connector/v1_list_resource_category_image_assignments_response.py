...
__all__ = ["V1ListResourceCategoryImageAssignmentsResponse", "ResourceCategoryImageAssignment"]
...
class V1ListResourceCategoryImageAssignmentsResponse(BaseModel):
    resource_category_image_assignments: List[ResourceCategoryImageAssignment] = FieldInfo(
        alias="ResourceCategoryImageAssignments"
    )
    ...
    assignment returned. This can be used in
    [Limitation](https://mews-systems.gitbook.io/connector-api/guidelines/pagination/#limitation)
    in a subsequent request to fetch the next batch of older resource category image
    assignments.
    """
